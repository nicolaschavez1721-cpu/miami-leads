...
# ─────────────────────────────────────────────
class ClerkScraper:
    # Updated URL - the portal moved to onlineservices subdomain
    ...
    async def _find_working_url(self, page) -> Optional[str]:
        """Try each search URL and return the first one that loads a form."""
    ...
    async def _safe_fill(self, page, selectors, value):
        ...
        for sel in selectors:
            try:
                ...
                if el:
                    await el.fill(value)
                    log.debug(f"Filled {sel} = {value}")
                    return True
            except Exception:
                pass
        log.debug(f"Could not fill any of {selectors} with {value}")
        return False
    ...
    async def _fill_form_and_search(self, page, doc_code: str) -> bool:
        ...
        inputs = {(i.get("id","") or i.get("name","")).lower(): (i.get("id") or i.get("name"))
                  for i in soup.find_all("input") if i.get("id") or i.get("name")}
        log.info(f"Available inputs: {list(inputs.keys())[:20]}")

        # Find date start field
        date_start_sel = None
        date_end_sel   = None
        doctype_sel    = None
        submit_sel     = None

        for key, real_id in inputs.items():
            sel = f"#{real_id}" if real_id else None
            if not sel:
                continue
            if any(x in key for x in ["startdate", "start_date", "datefrom", "date_from", "begindate", "fromdate", "recordedstart"]):
                date_start_sel = sel
            elif any(x in key for x in ["enddate", "end_date", "dateto", "date_to", "throughdate", "todate", "recordedend"]):
                date_end_sel = sel
            elif any(x in key for x in ["doctype", "doc_type", "instrumenttype", "recordtype"]):
                doctype_sel = sel
            elif any(x in key for x in ["search", "submit", "find", "btnname", "btnsearch"]):
                submit_sel = sel

        # Also check selects for doc type
        selects = soup.find_all("select")
        for sel_el in selects:
            sid = (sel_el.get("id","") or sel_el.get("name","")).lower()
            if any(x in sid for x in ["doctype", "doc_type", "instrumenttype", "recordtype"]):
                real_id = sel_el.get("id") or sel_el.get("name")
                doctype_sel = f"#{real_id}" if real_id else None

        log.info(f"date_start={date_start_sel}, date_end={date_end_sel}, doctype={doctype_sel}, submit={submit_sel}")

        # Fill dates
        if date_start_sel:
            await self._safe_fill(page, date_start_sel, self.date_from)
        if date_end_sel:
            await self._safe_fill(page, date_end_sel, self.date_to)

        # Fill doc type - try input fill or select option
        if doctype_sel:
            if doctype_sel.startswith("#"):
                el = await page.query_selector(doctype_sel)
                if el:
                    tag = await el.evaluate("el => el.tagName.toLowerCase()")
                    if tag == "select":
                        try:
                            await page.select_option(doctype_sel, value=doc_code)
                        except Exception:
                            try:
                                await page.select_option(doctype_sel, label=doc_code)
                            except Exception:
                                pass
                    else:
                        await self._safe_fill(page, doctype_sel, doc_code)
    ...
    async def _fetch_doc_type(self, page, doc_code: str) -> list[dict]:
        ...
        search_url = self.working_url or self.SEARCH
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                while True:
                    ...
                    # Debug: log page snippet
                    if page_num == 1:
                        snippet = soup.get_text()[:500].replace("\n", " ")
                        log.info(f"  Result page snippet: {snippet[:300]}")
                    ...
                    try:
                        ...
                        if "__doPostBack" in href:
                            ...
                            await page.click("a:has-text('Next')", timeout=5000)
                            await page.wait_for_load_state("networkidle", timeout=15000)
                        page_num += 1
    ...
    async def run(self) -> list[dict]:
        ...
        async with async_playwright() as pw:
            ...
            # Discover which URL works
            self.working_url = await self._find_working_url(page)
            if not self.working_url:
                log.error("Could not find a working Clerk portal URL — aborting scrape")
                await browser.close()
                return []

            log.info(f"Using Clerk URL: {self.working_url}")
