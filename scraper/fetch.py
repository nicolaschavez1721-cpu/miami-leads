name: Scrape Miami-Dade Leads

on:
  schedule:
    - cron: "0 7 * * *"   # 7:00 AM UTC daily (3:00 AM ET)
    ...
    steps:
      ...
      - name: Show result summary
        run: |
          ...
          print(f\"With address:  {d['with_address']}\")
          print(f\"Fetched at:    {d['fetched_at']}\")
          top = d['records'][:5]
          for r in top:
              print(f\"  [{r['score']}] {r['owner']} — {r['doc_type']} — {r['filed']}\")
          "
          else
            echo "No records.json found"
          fi
      ...
      - name: Commit and push results
        run: |
          ...
          RECORD_COUNT=$(python3 -c "import json; d=json.load(open('data/records.json')); print(d['total'])" 2>/dev/null || echo "0")
          SCRAPE_DATE=$(date +'%Y-%m-%d')
          git diff --staged --quiet || git commit -m "Auto-scrape ${SCRAPE_DATE} - ${RECORD_COUNT} records"
          git push
